    ...
    for episode in range(EPISODES):
        ...
        while not done:
            ...
            while w != END_TAG and len(outputs) < MAX_TARGET_LEN:
                curr_w_pos = torch.zeros([1, MAX_TARGET_LEN], dtype=torch.int32)
                curr_w_pos[:,0] = count
                dec_output, *_ = self.transformer.decoder(curr_w_seq, curr_w_pos, src_seq, enc_output)
                count += 1
                w_prob_b = F.log_softmax(self.transformer.tgt_word_prj(dec_output), dim=1)
        ...
        while len(history) >= BATCH_SIZE:
            ...
            for t in range(max_sentence_len):
                ...
                curr_w_pos = torch.zeros([1, MAX_TARGET_LEN], dtype=torch.int32)
                curr_w_pos[:,0] = t
                dec_output, *_ = self.transformer.decoder(prev_w_idx_b, dec_pos, src_seq, enc_output)
