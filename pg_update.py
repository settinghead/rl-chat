...
def main():
    ...
    sentimental_words_embd = get_GloVe_embeddings(sentimental_words, EMBEDDING_DIM)
        ...
        for (batch, (inp, targ)) in enumerate(dataset):
            with tf.GradientTape() as tape:
                ...
                for t in range(1, targ.shape[1]):
                    ...
                    onehot_labels = tf.keras.utils.to_categorical(y=actions, num_classes=len(targ_lang.word2idx))
                    
                    rewards = tf.convert_to_tensor(rewards, dtype="float32") * GAMMA
